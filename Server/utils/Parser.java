...
/**
 * Parser class provides utility methods to parse strings into structured
 * objects.
 * It is used to parse booking details and update types.
 */
public class Parser {

    /**
     * Parse a string containing booking details into a BookingDetails object.
     * 
     * @param bookingDetails The string containing booking details.
     * @return A BookingDetails object.
     */
    public static BookingDetails parseBookingDetails(String bookingDetails) {
        String[] details = bookingDetails.split(",");
        return new BookingDetails(details[0], DayOfWeek.valueOf(details[1]), Integer.parseInt(details[2]),
                Integer.parseInt(details[3]), Integer.parseInt(details[4]), Integer.parseInt(details[5]));
    }

    /**
     * Parse a string to extract the update type.
     * 
     * @param updateType The string containing the update type.
    ...
    /**
     * Parses a comma-separated string of booking details into two BookingDetails
     * objects:
     * one for the previous booking and one for the new booking.
     * 
     * Expected format:
     * facilityName,day1,startHour1,startMin1,endHour1,endMin1,day2,startHour2,startMin2,endHour2,endMin2
     * 
     * @param bookingDetailsStr The comma-separated string containing booking
     *                          details
     * @return An array of BookingDetails objects: [previousBooking, newBooking]
     * @throws IllegalArgumentException If the input string is invalid or cannot be
     *                                  parsed
     */
